...
const ProductList = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        flatProducts.forEach((product) => {
          const industryName = product.industry
          ...
          industryMap[industryName].categories[categoryName].products.push(
            product.name,
          );
        });
  ...
  const getCategoryIcon = (catName) => {
    ...
    if (cat && cat.icon) {
      const iconUrl = cat.icon.startsWith("/")
        ? `${BACKEND_URL}${cat.icon}`
        : cat.icon;
      return (
        <img src={iconUrl} alt={catName} className="w-8 h-8 object-contain" />
      );
    }
      ...
      industry.categories.forEach((category) => {
        ...
        category.products.forEach((productName) => {
          ...
          if (!grouped[category.name].some((p) => p.name === productName)) {
            ...
            const existing = grouped[category.name].find(
    ...
    return (
      <section className="font-primary mb-[75px]">
        <div className="container mx-auto px-10 md:px-18 xl:px-19 2xl:px-19">
          <div className="mb-12">
            <Skeleton variant="text-lg" className="mb-4 !w-80" />
            <Skeleton variant="text" className="!w-full max-w-7xl" />
          </div>
          <div className="flex flex-col lg:flex-row gap-5 xl:gap-4 mb-12">
            <Skeleton variant="text" className="!h-12 flex-1 !rounded-full" />
            <Skeleton
              variant="text"
              className="!h-12 !w-[200px] !rounded-full"
            />
            <Skeleton
              variant="text"
              className="!h-12 !w-[200px] !rounded-full"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            <Skeleton variant="product-card" count={15} />
          </div>
        </div>
      </section>
    );
      ...
      <div className="container mx-auto px-10 md:px-18 xl:px-19 2xl:px-19">
        ...
        >
          <h2 className="text-3xl md:text-[40px] font-bold text-primary">
            Chemical Solutions{" "}
            <span className="text-black font-bold">For Your Business</span>
          </h2>
          <p className="text-black mt-4 max-w-7xl font-normal italic text-[25px]">
            The following are chemical products that we are ready to distribute
            throughout Indonesia and overseas. For information on products not
            mentioned, please feel free to contact us.
          </p>
        </motion.div>
        ...
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-4 mb-12">
          <div className="flex-1 relative">
            ...
            <img
              src={SearchIcon}
              alt="Search"
              className="absolute right-6 top-1/2 -translate-y-1/2 w-5"
            />
          </div>
          ...
          <div className="relative">
            ...
            <img
              src={ArrowDown}
              alt="Arrow Down"
              className="absolute right-6 top-1/2 -translate-y-1/2 w-3 pointer-events-none"
            />
          </div>
